...
- `tag-taxonomy-rosters.md`'s real content no longer lives in this repo at all: it's declared in `dotty-private` and blueprint-applied to a machine-fixed path, resolved by a fifth key, `references.tag_taxonomy_rosters`. Both `lint.py` and `qa.py` take it as an explicit `--rosters-path` — never hardcode it, and never omit it (an unflagged call falls back to each script's own pre-key vault-relative default, which no longer resolves to real content). This repo's own `spec/tag-taxonomy-rosters.ci-placeholder.md` is the public-safe stand-in CI copies into the pre-key fallback path before running a check.
