...
- **Enforcer:** `/capture` (mid-session) and `/knowledge-layer` query-and-file invoked by `/session-closeout` Step 9 (boundary). Both emit candidates through the gatekeeper gatekeeper; neither routes candidates itself.
...
- **Enforcer:** gatekeeper, automated mode — router + gatekeeper for all candidate disposition.
...
| gatekeeper (automated mode) | §5 — automated capture; gatekeeper for all candidate disposition | Pending — reconcile to §5 |
